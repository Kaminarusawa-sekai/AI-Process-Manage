...
# 定义一个更详细的提示模板
template = """
作为一位资深的企业服务专家，您的一位同事刚刚完成了一项业务。这项业务是关于{process_classfication}的{process_name}，对流程执行可能存在的问题的分析是{reason_analysis}，原先的prompts是{original_prompts}
...
def get_process_suggestions_provide(input_varibles):
    
    # 使用链来获取响应"https://aichatlanba.openai.azure.com/openai/deployments/text-embedding-3-large/embeddings?api-version=2023-05-15"
    response = model_api.excute(template,input_varibles,["process_name","process_classfication","suggestions"],[str,str,str],["在这里填写流程的名字，即process_name","在这里填写流程的分类，即process_classfication","可能存在的建议是"])
